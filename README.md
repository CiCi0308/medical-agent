...
本地测试账号可通过注册创建；如果已有 demo 数据，可使用：
...
> 基于 FastAPI、LangChain Agent、Milvus 和医疗知识图谱，构建 Medical Agent 医疗用药安全系统。项目支持药品说明书 OCR 入库、个人病历/检查单病例档案管理、长期健康记忆、病例与药品说明书交叉分析，并通过 Agent 工具调用实现医疗知识图谱查询、药品说明书检索和病例定向检索。
