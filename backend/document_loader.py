...
class DocumentLoader:
    ...
    def _load_image_with_ocr(self, file_path: str, filename: str) -> list[dict]:
        api_key = os.getenv("ARK_API_KEY")
        base_url = os.getenv("BASE_URL")
        model = os.getenv("OCR_MODEL", "qwen-vl-plus")
        if not api_key or not base_url:
            raise ValueError("图片 OCR 需要配置 ARK_API_KEY 和 BASE_URL")
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": (
                                "请对这张医疗资料图片做 OCR，尽可能完整提取所有可见文字。"
                                "保留检查项目、数值、单位、参考范围、异常标记、日期和医院/科室信息。"
                                "只输出提取到的文字，不要诊断，不要解释。"
                            ),
                        },
                        {
        ...
        if not text:
            raise ValueError("图片 OCR 未提取到文字，请换一张更清晰的图片")
    ...
    def load_document(self, file_path: str, filename: str) -> list[dict]:
        ...
        if file_lower.endswith(".pdf"):
            ...
            try:
                return self._load_image_with_ocr(file_path, filename)
            except Exception as e:
                raise Exception(f"处理图片 OCR 失败: {str(e)}")
        else:
