...
from agent import chat_with_agent, chat_with_agent_stream, storage, update_health_profile_after_turn
...
from models import MedicalCase, User
...
DEFAULT_KNOWLEDGE_TYPE = "drug_instruction"
...
def _is_supported_document(filename: str) -> bool:
    ...
    return (
        ...
        or file_lower.endswith((".jpg", ".jpeg", ".png", ".webp"))
    )


def _normalize_knowledge_type(value: str | None) -> str:
    knowledge_type = (value or DEFAULT_KNOWLEDGE_TYPE).strip()
    if knowledge_type not in ALLOWED_KNOWLEDGE_TYPES:
        raise HTTPException(status_code=400, detail="知识类型不支持")
    return knowledge_type


def _normalize_case_id(knowledge_type: str, case_id: str | None) -> str:
    if knowledge_type != "personal_case":
        return ""
    return (case_id or "").strip()


def _attach_document_metadata(docs: list[dict], knowledge_type: str, owner_id: str, case_id: str = "") -> list[dict]:
    for doc in docs:
        doc["knowledge_type"] = knowledge_type
        doc["owner_id"] = owner_id
        doc["case_id"] = case_id or ""
    return docs


def _get_case(db: Session, user: User, case_id: str | None) -> MedicalCase | None:
    if not case_id:
        return None
    return (
        db.query(MedicalCase)
        .filter(MedicalCase.user_id == user.id, MedicalCase.case_id == case_id)
        .first()
    )


def _case_title_map(db: Session, user_id: int) -> dict[str, str]:
    rows = db.query(MedicalCase).filter(MedicalCase.user_id == user_id).all()
    return {row.case_id: row.title for row in rows}


def _format_case(row: MedicalCase) -> MedicalCaseInfo:
    return MedicalCaseInfo(
        case_id=row.case_id,
        title=row.title,
...
@router.get("/cases", response_model=MedicalCaseListResponse)
async def list_cases(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    cases = (
        db.query(MedicalCase)
        .filter(MedicalCase.user_id == current_user.id)
        .order_by(MedicalCase.updated_at.desc())
        .all()
    )
    return MedicalCaseListResponse(cases=[_format_case(row) for row in cases])


@router.post("/cases", response_model=MedicalCaseInfo)
async def create_case(
    request: MedicalCaseCreateRequest,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    title = (request.title or "").strip()
    if not title:
        raise HTTPException(status_code=400, detail="病例档案名称不能为空")
    row = MedicalCase(
...
def _process_upload_job(job_id: str, file_path: str, filename: str, knowledge_type: str, owner_id: str, case_id: str = "") -> None:
    ...
    try:
        ...
        _attach_document_metadata(new_docs, knowledge_type, owner_id, case_id)
        if knowledge_type == "personal_case":
            extracted_text = "\n\n".join([doc.get("text", "") for doc in new_docs if doc.get("text")])[:6000]
            update_health_profile_after_turn(
                owner_id,
                f"用户上传了个人病历/检查单《{filename}》，所属病例档案：{case_id or '未指定'}。OCR/解析内容如下：\n{extracted_text}",
                "已将上传资料纳入用户健康档案。",
            )
    ...
    try:
        ...
        try:
            results = milvus_manager.query(
                output_fields=["filename", "file_type", "knowledge_type", "owner_id", "case_id"],
                limit=10000,
            )
        except Exception:
            results = milvus_manager.query(
                output_fields=["filename", "file_type", "knowledge_type", "owner_id"],
                limit=10000,
            )
        db = SessionLocal()
        case_titles = {}
        try:
            ...
            for user in users:
                case_titles.update(_case_title_map(db, user.id))
        finally:
            db.close()
        ...
        for item in results:
            ...
            knowledge_type = item.get("knowledge_type") or DEFAULT_KNOWLEDGE_TYPE
            owner_id = item.get("owner_id") or ""
            raw_case_id = item.get("case_id") or ""
            case_id = raw_case_id if knowledge_type == "personal_case" else ""
            stats_key = (filename, knowledge_type, owner_id, case_id)
            if stats_key not in file_stats:
                file_stats[stats_key] = {
                    "filename": filename,
                    "file_type": file_type,
                    "knowledge_type": knowledge_type,
                    "owner_id": owner_id,
                    "case_id": case_id or None,
                    "case_title": case_titles.get(case_id),
                    "chunk_count": 0,
                }
            file_stats[stats_key]["chunk_count"] += 1
...
async def upload_document_async(
    ...
    knowledge_type: str = Form(DEFAULT_KNOWLEDGE_TYPE),
    case_id: str = Form(""),
    ...
    if not _is_supported_document(filename):
        raise HTTPException(status_code=400, detail="仅支持 PDF、Word、Excel 和图片文档")
    knowledge_type = _normalize_knowledge_type(knowledge_type)
    case_id = _normalize_case_id(knowledge_type, case_id)
    if case_id and not _get_case(db, current_user, case_id):
        raise HTTPException(status_code=404, detail="病例档案不存在")
...
async def upload_document(
    ...
    try:
        ...
        _attach_document_metadata(new_docs, knowledge_type, current_user.username, case_id)
        if knowledge_type == "personal_case":
            extracted_text = "\n\n".join([doc.get("text", "") for doc in new_docs if doc.get("text")])[:6000]
            update_health_profile_after_turn(
                current_user.username,
