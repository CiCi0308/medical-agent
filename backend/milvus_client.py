...
class MilvusManager:
    ...
    def __init__(self):
        ...
        self.timeout = float(os.getenv("MILVUS_TIMEOUT_SECONDS", "3"))
    ...
    def _is_closed_channel_error(exc: Exception) -> bool:
        text = str(exc).lower()
        return "closed channel" in text or "cannot invoke rpc on closed channel" in text
    ...
    def init_collection(self, dense_dim: int | None = None):
        ...
        def _init(client: MilvusClient) -> None:
            if not client.has_collection(self.collection_name):
                ...
                schema.add_field("knowledge_type", DataType.VARCHAR, max_length=50)
                schema.add_field("owner_id", DataType.VARCHAR, max_length=100)
                schema.add_field("case_id", DataType.VARCHAR, max_length=120)
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    ...
                    "knowledge_type": hit.get("knowledge_type", "drug_instruction"),
        ...
        for hits in results:
            for hit in hits:
                formatted_results.append({
                    ...
                    "knowledge_type": hit.get("entity", {}).get("knowledge_type", "drug_instruction"),
                    "owner_id": hit.get("entity", {}).get("owner_id", ""),
                    "case_id": hit.get("entity", {}).get("case_id", ""),
